...
const productInput = z.object({
  ...
  supplier_source_url: z.string().url().optional().or(z.literal("")).default(""), supplier_original_price_zar: z.number().min(0).nullable().optional(), supplier_name: z.string().trim().max(160).optional().default(""),
});
...
export const saveProduct = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d: unknown) => productInput.parse(d)).handler(async ({ data, context }) => {
  ...
  const source = { product_id: result.data.id, source_url: data.supplier_source_url || "", original_price_zar: data.supplier_original_price_zar ?? null, supplier_name: data.supplier_name || "", updated_at: new Date().toISOString(), updated_by: context.userId };
  const sourceResult = await db.from("product_supplier_sources").upsert(source, { onConflict: "product_id" });
  if (sourceResult.error) throw new Error("Product saved, but supplier source details could not be recorded.");
...
export const deleteProduct = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d: unknown) => z.object({ id: z.string().uuid() }).parse(d)).handler(async ({ data, context }) => {
  const { db } = await authorize(context, "ceo"); const { data: product } = await db.from("products").select("title").eq("id", data.id).maybeSingle();
...
export const updateOrderFulfillment = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d: unknown) => z.object({ id: z.string().uuid(), status: z.enum(["pending", "paid", "processing", "shipped", "delivered", "cancelled", "refunded"]) }).parse(d)).handler(async ({ data, context }) => {
  const { db } = await authorize(context, "ceo");
  const { data: order } = await db.from("orders").select("status, reference").eq("id", data.id).single(); if (!order) throw new Error("Order not found.");
  ...
  const { error } = await db.rpc("set_niberdealz_order_status", { p_order_id: data.id, p_status: data.status, p_actor_id: context.userId });
  if (error) throw new Error("Unable to update the order status.");
});

export const updateManualFulfilment = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d: unknown) => z.object({ id: z.string().uuid(), fulfilment_status: z.enum(["Pending Fulfilment", "Placed with supplier", "Fulfilled"]) }).parse(d)).handler(async ({ data, context }) => {
  const { db } = await authorize(context, "ceo");
  const { data: order } = await db.from("orders").select("reference, fulfilment_status").eq("id", data.id).single(); if (!order) throw new Error("Order not found.");
  const valid: Record<string, string[]> = { "Pending Fulfilment": ["Placed with supplier"], "Placed with supplier": ["Fulfilled", "Pending Fulfilment"], "Fulfilled": [] };
  if (order.fulfilment_status !== data.fulfilment_status && !valid[order.fulfilment_status]?.includes(data.fulfilment_status)) throw new Error("That fulfilment change is not allowed.");
  const { error } = await db.from("orders").update({ fulfilment_status: data.fulfilment_status, fulfilment_updated_at: new Date().toISOString(), fulfilment_updated_by: context.userId }).eq("id", data.id); if (error) throw new Error("Unable to update fulfilment status.");
  await audit(db, context, "manual_fulfilment_updated", "order", data.id, { reference: order.reference, from: order.fulfilment_status, to: data.fulfilment_status });
});

export const listFulfilmentQueue = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).handler(async ({ context }) => {
  const { db } = await authorize(context, "ceo");
  const { data, error } = await db.from("orders").select("id,reference,created_at,buyer_name,buyer_phone,delivery_method,delivery_tier,paxi_pickup_point,delivery_address,note,total_zar,payment_status,status,fulfilment_status,order_items(id,title,qty,size,color,comment,order_item_supplier_sources(source_url,original_price_zar,supplier_name))").order("created_at", { ascending: false }).limit(100);
  if (error) throw new Error("Unable to load fulfilment queue."); return data ?? [];
});
...
export const saveCategory = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d: unknown) => z.object({ id: z.string().uuid().optional(), name: z.string().trim().min(2).max(100), sort_order: z.number().int().min(0).max(1000).default(0), active: z.boolean().default(true) }).parse(d)).handler(async ({ data, context }) => {
  const { db } = await authorize(context, "ceo"); const result = data.id ? await db.from("store_categories").update({ name: data.name, sort_order: data.sort_order, active: data.active }).eq("id", data.id).select().single() : await db.from("store_categories").insert({ name: data.name, sort_order: data.sort_order, active: data.active }).select().single();
...
export const grantAdministratorByEmail = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d: unknown) => z.object({ email: z.string().trim().email() }).parse(d)).handler(async ({ data, context }) => {
  const { db } = await authorize(context, "ceo");
  const { data: users, error: lookupError } = await db.auth.admin.listUsers({ page: 1, perPage: 1000 });
  if (lookupError) throw new Error("Unable to find that account.");
  const user = (users?.users ?? []).find((item: any) => item.email?.toLowerCase() === data.email.toLowerCase());
  if (!user) throw new Error("That person needs a NiberDealz account before you can make them an admin.");
  const { data: roles } = await db.from("user_roles").select("role").eq("user_id", user.id);
  if ((roles ?? []).some((role: any) => role.role === "owner")) throw new Error("The CEO account cannot be changed here.");
...
const contactRequestInput = z.object({ name: z.string().trim().min(2).max(100), email: z.string().trim().email().max(254), topic: z.enum(["order", "product", "delivery", "return", "privacy", "other"]), message: z.string().trim().min(10).max(3000), preferred_contact: z.enum(["email", "whatsapp", "call"]) });
export const createContactRequest = createServerFn({ method: "POST" }).inputValidator((d: unknown) => contactRequestInput.parse(d)).handler(async ({ data }) => {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { error } = await (supabaseAdmin.from("contact_requests") as any).insert(data);
  if (error) throw new Error("We could not send your message. Please try again.");
  return { received: true };
});

export const listContactRequests = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).handler(async ({ context }) => {
  const { db } = await authorize(context, "ceo");
  const { data, error } = await db.from("contact_requests").select("id,name,email,topic,message,preferred_contact,created_at").order("created_at", { ascending: false }).limit(100);
  if (error) throw new Error("Unable to load private messages.");
  return data ?? [];
});

export const createNiberDealzOrder = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d: unknown) => z.object({ items: z.array(z.object({ product_id: z.string().uuid(), qty: z.number().int().min(1).max(50), size: z.string().nullable().optional(), color: z.string().nullable().optional(), comment: z.string().max(300).optional() })).min(1).max(30), buyer_name: z.string().trim().min(2).max(80), buyer_phone: z.string().trim().min(9).max(30), delivery_method: z.enum(["courier", "paxi", "pickup"]), delivery_address: z.string().trim().max(500).optional().default(""), note: z.string().trim().max(500).optional().default(""), discount_code: z.string().trim().max(40).optional().default(""), delivery_tier: z.enum(["courier", "pickup", "paxi_standard_5kg", "paxi_express_5kg", "paxi_standard_10kg", "paxi_express_10kg"]), paxi_pickup_point: z.string().trim().max(180).optional().default(""), terms_version: z.literal("2026-09") }).parse(d)).handler(async ({ data, context }) => {
  const { data: result, error } = await (context.supabase as any).rpc("create_niberdealz_order", { p_items: data.items, p_buyer_name: data.buyer_name, p_buyer_phone: data.buyer_phone, p_delivery_method: data.delivery_method, p_delivery_address: data.delivery_address, p_note: data.note, p_discount_code: data.discount_code, p_delivery_tier: data.delivery_tier, p_paxi_pickup_point: data.paxi_pickup_point, p_terms_version: data.terms_version });
  if (error) throw new Error("We could not create your order. Please review the checkout details and try again."); const order = Array.isArray(result) ? result[0] : result; if (!order?.order_id) throw new Error("Could not create your order."); return order;
});
